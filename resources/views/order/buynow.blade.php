<x-app-layout>
    <div class="container mx-auto px-4 py-8" style="margin-top: 60px;">
        <form action="{{ route('order.storebuynow') }}" method="POST" id="orderForm" enctype="multipart/form-data">
            @csrf
                        ...
                        <select name="payment_method_id" id="payment_method_id" required
                            ...
                            @foreach ($paymentMethods as $method)
                                <option value="{{ $method->id }}">
                                    {{ $method->provider }} - {{ $method->type_payment }} - {{ $method->no_account }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="p-6 border rounded-lg shadow-md bg-white">
                        ...
                        <div class="mb-4">
                            ...
                            <select
                                ...
                                <option value="">Pilih Alamat Asal</option>
                            </select>
                            @error('origin')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <select id="courier_id" name="courier_id" required
                                ...
                                @foreach($couriers as $courier)
                                    <option value="{{ $courier->id }}" data-requires-api="{{ $courier->requires_api }}">
                ...
                <div class="col-span-1 space-y-6">
                    ...
                    <div class="p-6 border rounded-lg shadow-md bg-white">
                        ...
                        <div class="mb-2">
                            ...
                            <p class="text-sm text-gray-600">Varian: {{ $selectedVarian}}</p>
                            <p class="text-sm text-gray-600">{{ $selectedQuantity }} x Rp.
            ...
            document.getElementById('pay-button').onclick = function () {
                snap.pay('<?=$snapToken?>', {
                    ...
                    onPending: function (result) {
                        submitOrder(result);
                        toastr.info('Pembayaran Anda sedang diproses. Silakan tunggu.', 'Pending');
                        window.location.href = '/order/thankyou';
                    },
                    onError: function (result) {
                        console.error('Payment Error:', result);
                        toastr.error('Terjadi kesalahan dalam pemrosesan pembayaran. Silakan coba lagi.', 'Error');
                ...
                fetch('/alamat-user')
                    ...
                    .then(data => {
                        ...
                        data.forEach(alamat => {
                            const option = document.createElement('option');
                            option.value = alamat.city_id;
                            option.textContent = `${alamat.nama_penerima} - ${alamat.alamat}`;
                            select.appendChild(option);
                        });
                    });
            }
